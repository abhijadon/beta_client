...
export default function DataTable({ config, extra = [] }) {
    ...
    const { crudContextAction } = useCrudContext();
    const { panel, collapsedBox, modal, editBox } = crudContextAction;
    ...
    const finalColumns = [
        ...
        ...dataTableColumns,
        {
            ...
            render: (_, record) => (
                <Dropdown
                    menu={{
                        items: items.filter((item) => item.key !== 'showDetails'),
                        onClick: ({ key }) => {
                            switch (key) {
                                case 'edit':
                                    handleEdit(record);
    ...
    const handleSearchChange = (e) => {
        ...
        if (value === '') {
            dispatcher(); // Reload all data when search input is cleared
        }
    };
    ...
    return (
        <>
            <Card className="rounded-3xl shadow-2xl drop-shadow-lg">
                <PageHeader
                    title={
                        <>
                            {DATATABLE_TITLE}
